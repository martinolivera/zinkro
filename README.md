...
Zinkro es un proyecto open source que propone un calendario alternativo: 13 meses de 28 días cada uno, más un Día Fuera del Tiempo (Día Zero el 20 de marzo), alineado con el equinoccio de marzo. El objetivo es reconectar el tiempo humano con los ritmos naturales y cósmicos, y facilitar la planificación anual.
...
- `tests/test_zinkro_leap.py`: Pruebas unitarias.
...
Zinkro is an open source project proposing an alternative calendar: 13 months of 28 days each, plus one Day Out of Time (Day Zero on March 20th), aligned with the March equinox. The goal is to reconnect human time with natural and cosmic rhythms, and to simplify annual planning.
...
- `tests/test_zinkro_leap.py`: Unit tests.
